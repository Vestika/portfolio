...
async def init_market_data_service(
    ...
    3. Build fetcher registry with rate limiters
    4. Create and start MarketDataReader (loads all data from MongoDB into RAM)
    5. Create and start MarketDataWriter (queries reader for state, starts workers)
    """
    if config is None:
